...
class CustomDataset(BaseDataset):
    ...
    @classmethod
    def make_dataset_index(cls, data_path, tokenizer, json_data_path):
        with open(os.path.join(json_data_path, "train.json"), "r") as fp:
            dacon_train = json.load(fp)
        with open(os.path.join(json_data_path, "val.json"), "r") as fp:
            dacon_val = json.load(fp)
        with open(os.path.join(json_data_path, "test.json"), "r") as fp:
            dacon_test = json.load(fp)

        all_answers = []

        for split, data in zip(["train", "val"], [dacon_train, dacon_val, dacon_test]):
            for i in data:
                all_answers.append(i["answer"])

        # all_answers를 빈도수에 따라 필터링하여 ans2label, label2ans를 만듭니다.
        all_answers = [normalize_word(word) for word in all_answers]
        counter = {k: v for k, v in Counter(all_answers).items() if v >= 5} # vqa dataset에 비해 데이터 수가 살짝 작으므로 9에서 5로 내립니다.
        ans2label = {k: i for i, k in enumerate(counter.keys())}
        label2ans = list(counter.keys())

        split2items = {}
        # score의 경우도 살짝 변경합니다. vqa셋과 형식이 조금 다르므로, 위에서 5번 이상 나타난 것과 대비하여 5번 이상 답이 나타나면 1, 그 밑은 빈도수를 줄여가면서 계산합니다.
        for split, data in zip(["train", "val", "test"], [dacon_train, dacon_val, dacon_test]):
            answer_count = {}
            items = []
            for i in data:
                tokens = tokenizer.tokenize(i['question'])
                token_ids = tokenizer.convert_tokens_to_ids(tokens)
                if split == 'train' or split == 'val':
                    ...
                    if answer not in ans2label:
                        continue
                    items.append({
                            "image_path": os.path.join('train', f"{i['image_id']}.jpg"), 
                            "text_segment": token_ids, 
                    ...
                    items.append({
                            "image_path": os.path.join('test', f"{i['image_id']}.jpg"), 
                            "text_segment": token_ids, 
                            "labels": '', 
                            "scores": '', 
                            "qid": int(i['question_id']), 
                        })
            split2items[split] = items
            _write_data_into_jsonl(items=items, jsonl_file=os.path.join(data_path, "vqa.%s.jsonl" % split))
